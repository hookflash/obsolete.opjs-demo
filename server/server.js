...
if (fs.existsSync(pathJoin(__dirname, "../.git"))) {
    MODE = "DEV";
}

var config = JSON.parse(fs.readFileSync(pathJoin(__dirname, "../config" + ((MODE==="dev")?".local":"") + ".json")));
...
function handler(req, res) {
    inviteRoute(req, res, function(err) {
        ...
        var pathname = url.parse(req.url, true);
        send(req, pathname.pathname).root(pathJoin(__dirname, '../client/public')).pipe(res);
    });
}
